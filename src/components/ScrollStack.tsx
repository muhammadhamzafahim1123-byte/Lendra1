...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '' }) => (
  <div
    className={`scroll-stack-card relative w-full h-auto min-h-[22rem] md:min-h-0 md:h-80 my-4 md:my-8 p-6 md:p-12 shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const progress = calculateProgress(scrollTop, triggerStart, pinEnd);
      ...
      if (hasChanged) {
        const transform = `translate3d(0, ${newTransform.translateY}px, 0) rotate(${newTransform.rotation}deg)`;
  ...
  return (
    ...
    >
      <div className="scroll-stack-inner pt-[8vh] pb-[12rem] min-h-screen">
        {children}
